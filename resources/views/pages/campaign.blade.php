        ...
        <div class="container">
            <div class="row">
                <div class="col-md-12 col-sm-12">
                    <h1 class="text-center" style="font-size: 48px; font-weight: 600; margin-top: 150px; color: white">Our Campaign's</h1>
                    <p class="text-center" style="font-size: 20px; color: white">Home <i class="fa fa-arrow-right"></i> Campaign</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="container">
        <div class="row">
            <div class="col-md-12 col-sm-12 mb-5">
                <h3 class="text-center p-5">Campaign</h3>
                @foreach($campaign as $campins)
                    <div class="col-md-4 col-sm-12 float-left mt-3">
                        <div class="card" style="border: 1px solid blue; height: 600px;">
                            <img src="{{ asset('campaign/images/'.$campins->image) }}" style="height: 200px; width: 100%;">
                                ...
                                <div class="table-responsive">
                                    <table class="table border-0 tab-pane">
                                        <tr style="height: 50px;">
                                            <th>Title</th>
                                            <td><span class="float-right">{{ $campins->title }}</span></td>
                                        </tr>
                                        <tr style="height: 50px;">
                                            <th>Location</th>
                                            <td>
                                                <span class="float-right text-justify">{{ $campins->location  }}</span>
                                            </td>
                                        </tr>
                                        <tr style="height: 50px;">
                                            <th>Start Date</th>
                                            <td><span class="float-right">{{ $campins->start_date }}</span></td>
                                        </tr>
                                        <tr style="height: 50px;">
                                            <th>End Date</th>
                                            <td><span class="float-right">{{ $campins->end_date }}</span></td>
                                        </tr>
                                    </table>
                                </div>
                            </div>
                            <div class="card-footer">
                                <a class="btn btn-info float-right" href="{{ route('pages.campgain_details', $campins->id) }}">View More</a>
                            </div>
                        </div>
                    </div>
                @endforeach
