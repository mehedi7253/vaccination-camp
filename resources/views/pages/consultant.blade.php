        ...
        <div class="container">
            <div class="row">
                <div class="col-md-12 col-sm-12">
                    <h1 class="text-center" style="font-size: 48px; font-weight: 600; margin-top: 150px; color: white">Our Consultant's</h1>
                    <p class="text-center" style="font-size: 20px; color: white">Home <i class="fa fa-arrow-right"></i> Consultant</p>
                </div>
            </div>
        </div>
    </div>
...
<section class="consultant_body" style="background-color: #d4edda">
    <div class="container">
        <div class="row">
            @foreach($consultant as $consultants)
            <div  class="col-xs-12 col-sm-6 col-md-3 mt-5 mb-3">
                <div class="card">
                    <div class="card-body text-center">
                        <p><img class="img-fluid" src="{{ asset('consultant/images/'.$consultants->image) }}" alt="card image" style=" height: 150px; width: 150px; border-radius: 50%;"></p>
                        <h2 class="text-capitalize font-weight-normal">{{ $consultants->name }}</h2>
                        <p class="text-capitalize">{{ $consultants->designation }}</p>
                        <p class="text-capitalize">{{ $consultants->phone }}</p>
                        <p class="text-center">{{ $consultants->email }}</p>
                        <a href="{{ route('pages.consultant_profile',$consultants->id) }}" class="btn btn-primary btn-block">View More</a>
                    </div>
                </div>
            </div>
            @endforeach
